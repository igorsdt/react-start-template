...
export const getColorContrastValue = ([red, green, blue]:RGBArrType) =>
...
export const hex2rgb = (color:string):RGBArrType => {
  checkColor(color);
...
export const toStringArray = (arr:ToStringArrayParamsType):string[] => arr.map(({ value, number }) => `${value}_${number}`);
...
type AccelerometerObj = {
  [key: number]: Omit<Сustomer, 'id'>
}

export const transformCustomers = (customers:Сustomer[]) => {
  return customers.reduce((acc:AccelerometerObj, customer:Сustomer) => {
